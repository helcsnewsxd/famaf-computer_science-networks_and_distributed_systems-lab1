...
def connect_to_server(server_name):
    ...
    Traceback (most recent call last):
       ...
    gaierror: [Errno -5] No address associated with hostnames
    ...
    # Buscar direccion ip
    ip_address=socket.gethostbyname(server_name) # Función obtiene IP address a través del server_name
    sys.stderr.write("Contactando al servidor en %s...\n" % ip_address)
    s = socket.socket(socket.AF_INET,socket.SOCK_STREAM) # Creamos socket
 
    s.connect((ip_address,80)) # Nos conectamos al servidor usando la IP obtenida con el socket.
    return s
